import {BarcodeType, BarcodeTypeDigits, BarcodeTypeSplits} from "../types";


const barcodeDigits:BarcodeTypeDigits = {
    ['GTIN-8']: 8,
    ['GTIN-12']: 12,
    ['GTIN-13']: 13,
    ['GTIN-14']: 14,
...
const barcodeSplits:BarcodeTypeSplits = {
    ['GTIN-12']: /(\d)(\d{5})(\d{5})(\d)/,
    ['GTIN-13']: /(\d)(\d{6})(\d{6})/,
    ['GTIN-14']: /(\d)(\d{2})(\d{5})(\d{5})(\d)/,
}

function trimUPC(digits:string):string {
    return digits.trim().replace(/\D/g, '');
}


function parseCheckDigit(str: string, format: BarcodeType = 'GTIN-12'):string {
    const digitSum = [0, 0];
    let digits = trimUPC(str);
    const stdDigits = barcodeDigits[format];
    if (!digits || digits.length < (stdDigits - 1) || digits.length > stdDigits) {
        return '';
    } else if (digits.length === stdDigits) {
...
function parseUPCA(upc:string):string {
...
function parseFormat(str:string):BarcodeType|null {
    const digits = trimUPC(str);
    switch (digits.length) {
    case barcodeDigits["GTIN-8"]:
    case barcodeDigits["GTIN-8"] - 1:
        return 'GTIN-8';
...
export function formatUPC(upc:string) {
    const format = parseFormat(upc);
    if (!format) {
        return upc;
    }
